...
You are given a rotated, sorted array of distinct integers. The array was originally sorted in
ascending order,
but then rotated at an unknown pivot, meaning part of the array may still be in order, but the
overall array is no
longer fully sorted.

Your task is to search for a specific target value within this array and return its index. If the
target value is
...
We can solve this problem using a modified version of the binary search algorithm. Although the
array is rotated,
...
5. Continue narrowing down the search range until the target is found or the search space is
exhausted.

The algorithm efficiently searches for the target in O(log n) time by repeatedly halving the search
space, taking
