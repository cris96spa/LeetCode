...
class Solution:
    ...
    - Then, for each of the remaining elements in the array, we check if it is larger than the
      smallest element in the heap (the root). If it is, we replace the root with the current
      element.
    - At the end, the root of the heap contains the kth largest element.

    This approach ensures that we maintain a heap of only `k` elements, optimizing the time
    complexity.

    Time Complexity:
    - Initial heap construction takes O(k).
    - For the remaining `n-k` elements, each insertion into the heap takes O(log k).
    - Therefore, the overall time complexity is O(n log k), where n is the length of `nums` and k is
    the given input.
