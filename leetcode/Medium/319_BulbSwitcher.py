...
The key observation is that a bulb will be toggled in each round where its index is a divisor of the
round number.
For example, bulb 6 will be toggled in rounds 1, 2, 3, and 6. Bulbs that have an odd number of
divisors will remain on,
and this happens only for perfect squares (because they have an odd number of divisors). Therefore,
...
Input: n = 3
Output: 1
Explanation: At first, the three bulbs are [off, off, off]. After the first round, the three bulbs
are [on, on, on].
After the second round, the bulbs become [on, off, on]. After the third round, they become [on, off,
off].
